...
inline uint16_t ProfileGetTimerIndex(ProfileToken t)
{
	return (t & 0xffff);
}
inline uint64_t ProfileGetGroupMask(ProfileToken t)
{
	return ((t >> 16)&PROFILE_GROUP_MASK_ALL);
}
inline ProfileToken ProfileMakeToken(uint64_t nGroupMask, uint16_t nTimer)
{
...
typedef void ProfileWriteCallback(void* Handle, size_t size, const char* p_data);
