...
{
	return gResourceDirectoryDefaults[resourceDir];
}
...
bool fsCopyFile(const Path* sourcePath, const Path* destinationPath, bool overwriteIfExists)
{
	...
	{
		if ( !overwriteIfExists && fsFileExists(destinationPath) )
		{
			return false;
		}
		...
		if ( fileSize <= 0 )
		{
			...
			{
				uint8_t byte = 0;
				if ( !success || fsReadFromStream(toRead, &byte, sizeof(uint8_t)) != sizeof(uint8_t) )
				{
					success = false;
				}

				if ( !success || fsWriteToStream(toWrite, &byte, sizeof(uint8_t)) != sizeof(uint8_t) )
				{
					success = false;
				}
			}
		...
		else
		{
			...
			if ( fsReadFromStream(toRead, tempBuffer, fileSize) != fileSize )
			{
				success = false;
			}
			if ( !success || fsWriteToStream(toWrite, tempBuffer, fileSize) != fileSize )
			{
				success = false;
			}
...
{
	return fsReadFromStreamType<float2>(stream);
}

float3 fsReadFromStreamFloat3(FileStream* stream)
{
	return fsReadFromStreamType<float3>(stream);
}
...
{
	return fsReadFromStreamType<float4>(stream);
}
...
{
	return fsWriteToStreamType(stream, value);
}
