...
template <typename T>
static inline T withUTF16Path(const Path* path, stl::function<T(const wchar_t*)> function)
{
...
FileWatcher* fsCreateFileWatcher(const Path* path, FileWatcherEventMask eventMask, FileWatcherCallback callback)
{
	...
	watcher->mThreadDesc.func = fswThreadFunc;
	watcher->mThreadDesc.data = watcher;

	watcher->mThread = CreateThread(&watcher->mThreadDesc);
