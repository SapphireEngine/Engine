...
stl::string fsReadFromStreamSTLString(FileStream* stream);
stl::string fsReadFromStreamSTLLine(FileStream* stream);

/// Forms an `stl::string` from `pathComponent`
inline stl::string fsPathComponentToString(PathComponent pathComponent)
{
    return stl::string(pathComponent.buffer, pathComponent.length);
}

/// Returns `path`'s file name and extension as a string. The return value is guaranteed to live for as long as `path` lives.
stl::string fsGetPathFileNameAndExtension(const Path* path);

/// Collects the results of calling `fsEnumerateFilesWithExtension` with `directory` and `extension` into an `stl::vector<PathHandle>`.
stl::vector<PathHandle> fsGetFilesInDirectory(const Path* directory);

/// Collects the results of calling `fsEnumerateFilesWithExtension` with `directory` and `extension` into an `stl::vector<PathHandle>`.
stl::vector<PathHandle> fsGetFilesWithExtension(const Path* directory, const char* extension);

/// Collects the results of calling `fsEnumerateSubdirectories` with `directory` into an `stl::vector<PathHandle>`.
stl::vector<PathHandle> fsGetSubDirectories(const Path* directory);
...
{
    template <>
    struct hash<se::PathHandle>
    {
        std::size_t inline operator()(const se::PathHandle& k) const
        {
            const char* s = se::fsGetPathAsNativeString(k);
            const se::FileSystem* fs = se::fsGetPathFileSystem(k);

            size_t h = se::stl::hash<const se::FileSystem*>()(fs);
