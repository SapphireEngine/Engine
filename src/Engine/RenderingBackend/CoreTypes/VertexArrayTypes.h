...
struct VertexAttribute final
{
	...
	// Data source
	uint32_t			  inputSlot;				// Index of the vertex input slot to use (see "VertexArrayVertexBuffer")
	...
	uint32_t			  instancesPerElement;		/**< Number of instances to draw with the same data before advancing in the buffer by one element.
														 ...
														 Instanced arrays is a shader model 3 feature, only supported if "Capabilities::instancedArrays" is true.
...
*
*  @see
*    - "IVertexArray" class documentation
*/
#pragma pack(push)
#pragma pack(1)
...
{
	IVertexBuffer* vertexBuffer;	// Vertex buffer used at this vertex input slot (vertex array instances keep a reference to the vertex buffers used by the vertex array attributes, see "IRhi::createVertexArray()" for details)
};
