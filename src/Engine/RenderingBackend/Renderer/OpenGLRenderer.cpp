...
{
	SE_NAMESPACE_WND::WindowDesc desc = window.getDesc();
...
{
	OpenGLDesc ogldesc;
	mOGLState = createContext(&window, ogldesc);
	setContext(mOGLState);
	if ( !gladLoadGL() )
	{
		// Failed
...
{
	unsetContext(mOGLState);
	destroyContext(mOGLState);
}
