...
typedef struct UpdateRequest
{
	...
	UpdateRequest(Buffer* buf) : mType(UPDATE_REQUEST_UPDATE_RESOURCE_STATE)
	{
		buffer = buf; texture = NULL;
	}
	UpdateRequest(Texture* tex) : mType(UPDATE_REQUEST_UPDATE_RESOURCE_STATE)
	{
		texture = tex; buffer = NULL;
	}
...
static void addResourceLoader(Renderer* pRenderer, ResourceLoaderDesc* pDesc, ResourceLoader** ppLoader)
{
	...
	pLoader->mThreadDesc.func = streamerThreadFunc;
	pLoader->mThreadDesc.data = pLoader;
	...
	pLoader->mThread = CreateThread(&pLoader->mThreadDesc);
